...
import { SITE_THEMES, themeCssVars } from '../themes/siteThemes'
...
/** 规范链接 <link rel="canonical"> 注入（缺失则移除，避免残留） */
...
function SitePage() {
  // 站点级主题 + 模板（发布者后台设定，访客可本地覆盖）
  const { theme, template, siteTitle, siteTagline, setLocalTheme } = useSiteTheme()
  const brand = siteTitle || 'LightPress'
  const tagline = siteTagline || '专注内容的现代发布平台'
  ...
  // 首页精选位：优先展示 featured 文章，否则回退最新一篇
  const hero = useMemo(() => list.find((a) => a.featured) ?? list[0], [list])

  // 首页级 OG/Twitter Card meta 注入（文章详情打开时由下方 effect 覆盖）
  useSeo({
    title: `${brand} · ${tagline}`,
    description: tagline,
  ...
  /** 点击标签：跳转到独立语义化标签路由 /tag/{slug}（P3，替代页内 ?tag= 筛选） */
  const pickTag = (tg: string) => {
    setOpenId(null)
    navigate({ to: '/tag/$slug', params: { slug: encodeURIComponent(tg) } })
  }

  /** 点击作者：跳转到独立作者页 /author/{name} */
  const pickAuthor = (name?: string) => {
    if (!name) return
    setOpenId(null)
    navigate({ to: '/author/$name', params: { name: encodeURIComponent(name) } })
  }
  ...
  useEffect(() => {
    ...
    setMeta('property', 'og:title', `${seoTitle} · ${brand}`)
    ...
    // 规范链接：自定义 canonical_url 优先，否则用公开文章 URL
    const canonical = current.canonical_url?.trim()
      ? current.canonical_url.trim()
      : `${window.location.origin}/read/${keyOf(current)}`
      ...
      >
        <div className="mx-auto max-w-5xl px-5 sm:px-8 h-14 flex items-center gap-3">
          ...
          >
            ...
            <span className="font-semibold tracking-tight truncate">{brand}</span>
            <span
        ...
        <>
          ...
          >
            <div className="mx-auto max-w-5xl flex flex-col items-start gap-3">
              ...
              <h1 className={template.hero === 'large' ? 'text-4xl sm:text-[3.4rem] leading-[1.1] font-bold tracking-tight max-w-3xl' : 'text-3xl sm:text-[2.6rem] leading-tight font-bold tracking-tight max-w-3xl'}>
                {hero.title}
              </h1>
              <p
              ...
              <div className="flex items-center gap-3 mt-2 text-xs" style={{ color: theme.vars.muted }}>
                <button
                  onClick={() => pickAuthor(hero.author)}
                ...
                <span>·</span>
                <span>{fmtDate(hero.published_at || hero.updated_at)}</span>
              </div>
              <button
                onClick={() => setOpenId(keyOf(hero))}
          ...
          <main className="mx-auto max-w-5xl px-5 sm:px-8 pb-20 -mt-8 relative z-10">
            <div className={template.gridCols}>
              {filtered.map((a) => {
                const cover = coverOf(a)
                const tags = parseTags(a.tags)
                if (template.card === 'text') {
                  return (
                    <article
                      key={a.id}
                      onClick={() => setOpenId(keyOf(a))}
                      className="group rounded-xl border p-4 cursor-pointer transition-colors hover:opacity-90 flex flex-col gap-1"
                      style={{ background: theme.vars.surface, borderColor: theme.vars.border }}
                    >
                      <div className="flex items-baseline gap-3">
                        <h3 className="font-semibold leading-snug flex-1">{a.title}</h3>
                        <span className="text-[11px] whitespace-nowrap" style={{ color: theme.vars.muted }}>
                          {fmtDate(a.published_at || a.updated_at)}
                        </span>
                      </div>
                      <p className="text-xs line-clamp-2" style={{ color: theme.vars.muted }}>{excerpt(a)}</p>
                      <div className="flex items-center gap-2 text-[11px] flex-wrap" style={{ color: theme.vars.muted }}>
                        {a.author ? (
                          <button onClick={(e) => { e.stopPropagation(); pickAuthor(a.author) }} className="hover:underline">{a.author}</button>
                        ) : null}
                        {tags.slice(0, 2).map((tg) => (
                          <button key={tg} onClick={(e) => { e.stopPropagation(); pickTag(tg) }} className="px-1.5 py-0.5 rounded-full hover:opacity-80" style={{ background: theme.vars.surfaceAlt, color: theme.vars.accent }}>{tg}</button>
                        ))}
                      </div>
                    </article>
                  )
                  ...
                  >
                    ...
                    >
                      ...
                      {a.visibility && a.visibility !== 'public' && (
                        <span
                          className="absolute top-2 right-2 grid place-items-center w-7 h-7 rounded-full bg-black/45 text-white text-sm backdrop-blur"
                          title={a.visibility === 'paid' ? '付费会员专享' : '会员专享'}
                        >
                          {a.visibility === 'paid' ? '💎' : '🔒'}
                        </span>
                      )}
      ...
      {phase === 'ready' && current && (
        <article className="mx-auto max-w-[720px] px-5 sm:px-8 pt-8 pb-24">
          ...
          >
            ...
            <span>
              <button onClick={() => pickAuthor(current.author)} className="hover:underline" title="查看该作者文章">
                {current.author || '佚名'}
              </button>
            </span>
            <span>·</span>
          ...
          {coverOf(current) && (
            <img
              src={coverOf(current)!}
              alt={current.title || ''}
              srcSet={buildSrcset(coverOf(current)) || undefined}
          ...
          {current.visibility && current.visibility !== 'public' && (
            <div
              className="mt-6 rounded-2xl border p-5 flex items-center justify-between gap-4"
              style={{ borderColor: theme.vars.border, background: theme.vars.surface }}
            >
              <div className="min-w-0">
                <p className="font-medium">
                  {current.visibility === 'paid' ? '付费会员专享全文' : '会员专享全文'}
                </p>
                <p className="text-xs mt-1" style={{ color: theme.vars.muted }}>
                  登录或订阅后即可阅读完整内容
                </p>
              </div>
              <a
                href={`/read/${encodeURIComponent(keyOf(current))}`}
